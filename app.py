...
<style>
    ...
    .disclaimer-banner strong { color: #533f03; }
    .range-card {
        background: white;
        border-radius: 12px;
        padding: 20px;
...
def train_models(df):
    ...
    preds = {
        ...
        "GB Tuned":              gb_tuned.predict(X_test),
        "XGB Tuned":             xgb_tuned.predict(X_test),
    }

    # ── Per-model RMSE (for showing alongside predictions) ───────────────────
    model_rmse = {}
    for name, p in preds.items():
        model_rmse[name] = float(np.sqrt(mean_squared_error(y_test, p)))
    ...
    return {
        ...
        "models": {"lr": lr, "lasso": lasso, "dt": dt, "knn": knn,
                   ...
                   "gb_tuned": gb_tuned, "xgb_tuned": xgb_tuned},
        "preds": preds,
...
# ─── Load ─────────────────────────────────────────────────────────────────────
with st.spinner("Loading data, training & tuning models (first load takes ~60s)…"):
    df = load_data()
...
<div class='disclaimer-banner'>
  ⚠️ <strong>For Educational Purposes Only.</strong>
  This app demonstrates a machine learning pipeline on the 1990 California Housing census dataset.
  Predictions are illustrative — do <strong>not</strong> use them for real estate or financial decisions.
  Model accuracy is limited by the dataset's features and age.
</div>
""", unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    ensemble_models = ["Voting", "Bagging", "Random Forest", "Gradient Boosting", "AdaBoost", "XGBoost", "Stacking", "GB Tuned", "XGB Tuned"]
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown("""
    <div class='disclaimer-banner'>
      📌 <strong>How to read this prediction:</strong>
      The model gives a <em>point estimate</em> plus a ±RMSE error band — meaning the true value
      typically falls within that range. A lower RMSE = more confident model.
      All predictions are for <strong>educational purposes only</strong>.
    </div>
    """, unsafe_allow_html=True)
    ...
    model_options = {
        "🏆 GB Tuned (Best)":       ("gb_tuned",  True),
        "🥈 XGB Tuned":             ("xgb_tuned", True),
        "Gradient Boosting":        ("gb",         True),
        "XGBoost":                  ("xgb",        True),
        "Random Forest":            ("rf",         False),
        "Linear Regression":        ("lr",         False),
    }
    model_sel = st.selectbox("Choose prediction model", list(model_options.keys()))
    model_key, use_raw = model_options[model_sel]
    mdl  = bundle["models"][model_key]
    rmse = bundle["model_rmse"].get(
        model_sel.replace("🏆 ", "").replace("🥈 ", "").replace(" (Best)", ""),
        bundle["model_rmse"].get("GB Tuned" if "gb_tuned" in model_key else "XGB Tuned",
        bundle["model_rmse"].get("Gradient Boosting", 83000))
    )
    # resolve rmse cleanly
    label_clean = {"gb_tuned": "GB Tuned", "xgb_tuned": "XGB Tuned",
                   "gb": "Gradient Boosting", "xgb": "XGBoost",
                   "rf": "Random Forest", "lr": "Linear Regression"}[model_key]
    rmse = bundle["model_rmse"].get(label_clean, 83000)
    r2   = r2_score(bundle["y_test"], bundle["preds"].get(label_clean, bundle["preds"]["Gradient Boosting"]))

    if st.button("🏠 Predict House Value", type="primary"):
        pred = mdl.predict(input_df if use_raw else input_scaled)[0]
        pred = max(0, pred)
        low  = max(0, pred - rmse)
        high = pred + rmse
        ...
        <div style='text-align:center; background: linear-gradient(135deg,#1f77b4,#0d4f8c);
                    border-radius:16px; padding:32px; color:white; margin-bottom:16px;'>
            <p style='margin:0; font-size:0.9rem; opacity:0.75; letter-spacing:1px;'>
            ...
            <p style='margin:0; font-size:1rem; opacity:0.85;'>
                Predicted by <strong>{model_sel}</strong>
            </p>
        </div>
        """, unsafe_allow_html=True)
        ...
        c1.markdown(f"""<div class='range-card'>
            <h3 style='color:#dc3545; font-size:0.8rem; margin:0'>⬇ LOW ESTIMATE</h3>
            <p style='font-size:1.5rem; font-weight:700; color:#dc3545; margin:4px 0'>${low:,.0f}</p>
            <p style='font-size:0.75rem; color:#6c757d; margin:0'>Point estimate − RMSE</p>
        </div>""", unsafe_allow_html=True)

        c2.markdown(f"""<div class='range-card' style='border:2px solid #1f77b4;'>
            <h3 style='color:#1f77b4; font-size:0.8rem; margin:0'>🎯 POINT ESTIMATE</h3>
            <p style='font-size:1.5rem; font-weight:700; color:#1f77b4; margin:4px 0'>${pred:,.0f}</p>
            <p style='font-size:0.75rem; color:#6c757d; margin:0'>Model prediction</p>
        </div>""", unsafe_allow_html=True)

        c3.markdown(f"""<div class='range-card'>
            <h3 style='color:#28a745; font-size:0.8rem; margin:0'>⬆ HIGH ESTIMATE</h3>
            <p style='font-size:1.5rem; font-weight:700; color:#28a745; margin:4px 0'>${high:,.0f}</p>
            <p style='font-size:0.75rem; color:#6c757d; margin:0'>Point estimate + RMSE</p>
        </div>""", unsafe_allow_html=True)
        ...
        st.markdown("<div class='section-header'>Model Quality for This Prediction</div>", unsafe_allow_html=True)
        mc1, mc2, mc3 = st.columns(3)
        mc1.markdown(f"""<div class='metric-card'>
            <h3>R² Score</h3><p>{r2:.4f}</p>
            <small style='color:#6c757d'>1.0 = perfect; explains {r2*100:.1f}% of variance</small>
        </div>""", unsafe_allow_html=True)
        mc2.markdown(f"""<div class='metric-card'>
            <h3>Typical Error (RMSE)</h3><p>±${rmse:,.0f}</p>
            <small style='color:#6c757d'>Prediction is usually within this range</small>
        </div>""", unsafe_allow_html=True)
        ...
        mc3.markdown(f"""<div class='metric-card'>
            <h3>vs. Dataset Average</h3>
            <p style='color:{"#28a745" if pct_diff > 0 else "#dc3545"}'>{abs(pct_diff):.1f}% {direction}</p>
            <small style='color:#6c757d'>Dataset avg: ${avg:,.0f}</small>
        </div>""", unsafe_allow_html=True)

        # ── Visual range bar ──────────────────────────────────────────────────
        st.markdown("<div class='section-header'>Prediction Range Visualised</div>", unsafe_allow_html=True)
        fig, ax = plt.subplots(figsize=(10, 2))
        ax.barh(["Estimate"], [high - low], left=[low], color="#1f77b4", alpha=0.3, height=0.4, label="±RMSE range")
        ax.axvline(pred, color="#1f77b4", lw=2.5, label=f"Prediction: ${pred:,.0f}")
        ax.axvline(avg,  color="#ff7f0e", lw=1.5, linestyle="--", label=f"Dataset avg: ${avg:,.0f}")
        ax.set_xlabel("House Value ($)")
        ax.xaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: f"${x:,.0f}"))
        ax.legend(loc="upper right", fontsize=9)
        ax.set_title("Point Estimate with ±RMSE Error Band", fontweight="bold")
        plt.tight_layout()
        st.pyplot(fig); plt.close()

        # ── Tuning info ───────────────────────────────────────────────────────
        if "Tuned" in model_sel:
            key = "gb_best_params" if "GB" in model_sel else "xgb_best_params"
            st.info(f"🔧 **GridSearchCV best params for {model_sel}:** `{bundle[key]}`")
